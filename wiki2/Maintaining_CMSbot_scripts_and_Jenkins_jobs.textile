...
h1. Maintaining the LArSoft for of CMSbot scripts and Jenkins Github Integration CI jobs at FNAL

h2. The CMSbot scripts for LArSoft repos
...
h2. The GitHub webhook processing cgi script website
...
h2. * Scripts used to set up labels for pull requests/issues and the set up web hooks for GitHub repo(s).
...
h2. The Jenkins jobs that run the CI requests
