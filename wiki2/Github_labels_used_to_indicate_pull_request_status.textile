...
h1. Github labels used as status indicators
...
|{background:#F00}.  %{color:white}-rejected% | the CI process failed |

h2. Signature category labels

* The signature category is assigned depending on what files are changed in the pull request:
|_. signature prefix |_. files/dirs changed |
| L1 | all files |
...
|{background:#F00}.  %{color:white}-rejected% | PR has been rejected PR by L1 or L2 manager |

h2. Special labels

Two special labels/colors are added to a pull request to indicate the overall status.
|_. label (color) |_. overall status |
...
|_. label (color) |_. status|
|{background:#0000CD}. %{color:white}requires external% | requires a PR from another LArSoft repo |
