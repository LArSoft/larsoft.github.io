...
h1. Rerun part or all a job on an output file of that job

Say you have run the configuration @myJob.fcl@, which has produced the output file @myOutput.root@.
If you decide you want to try and run @myJob.fcl@ again, but for any reason you don&#39;t want to rerun on the original input file (because it&#39;s lost, or because you don&#39;t want to actually rerun all the modules in @myJob.fcl@ but only part of them), you will need to start with @myOutput.root@ as new _input_ file.

The new configuration can be _almost_ a copy of @myJob.fcl@, but you have to tell _art_ to completely ignore the data products you know you are going to regenerate.
Two options:
...
bq. *Note:* these procedures do not guarantee that the random number sequences will be the same as in the original job. To achieve that, additional effort might be needed.

h2. Regenerate all the data products from a job
...
This will drop _from the input_ all data products produced with a process name @<ProcessName>@.

bq. *Note*: even so, _art_ will remember that @<ProcessName>@ happened already and it will forbid the same process name to be used again. For this reason, @process_name@ _must_ be specified.

You will also want to make sure that:

* the output file name is different than before (you can use _art_ command line argument @--output@ or specify the output file name in the new FHiCL file)
...
h2. Regenerate the data products from selected modules in a job
...
We assume we want to rerun the modules with _labels_ @module1@, @module2@, up to @moduleN@

The key ingredient is an @inputCommands@ line like:
<pre>source.inputCommands: [
  "keep *",
...
This will drop _from the input_ all data products produced by modules with labels @module1@, @module2@, etc., with a process name @<ProcessName>@.
...
physics.reco: [ "module1", "module2", ... , "moduleN" ]</pre>


h2. The _art_ branch name

The specification of _art_ branches used in @inputCommands@ of @RootInput@ (and also in @outputCommands@ from @RootOutput@ are in the form:
<pre><ClassName>_<ModuleLabel>_<InstanceLabel>_<ProcessName></pre>
where:
...
<pre>root -lb myOutput.root
Events->GetListOfBranches()->ls()</pre>
shows all data product branches, while
<pre>Events->GetListOfBranches()->ls("*_<ProcessName>.")</pre>
will show only the data products produced by @<ProcessName>@ (note the dot @.@ at the end of the string).

--- 

*%{color: red}These instructions haven&#39;t been tested yet.%*
_For comments, write to "Gianluca Petrillo":mailto:petrillo@fnal.gov ._
