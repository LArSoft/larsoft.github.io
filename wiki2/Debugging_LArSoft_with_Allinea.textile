...
h1. Debugging LArSoft with Allinea
...
Fermilab has a few licenses available: see "details below":https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/Debugging_LArSoft_with_Allinea/#Licenses
...
<pre>ups list -aK+ allinea
setup allinea <version></pre>
The above is obsolete. Now you just need to setup forge_tools:
<pre>setup forge_tools</pre>
...
There are a few things in _art_ that are multi-threaded, such as the messaging system and interaction with the SQLite database when running time or memory profiling services, though your program should be able to run single-threaded as well. Feel free to experiment with it.
...
h3. Searching extra directories
...
bq. Please make sure you don&#39;t leave an Allinea instance open, since that could prevent other people from using it.
