...
h1. Which types should go in my C++ function arguments?

The following apply to pure functions. If you are dealing with class methods, some points might be different.

h2(count). Decorators: pointers, references, constant references
...
<pre><code class="cpp">std::vector<float> fillRedOrBlack(std::vector<float> const&amp; v, bool red);</code></pre>
that gets used as:
<pre><code class="cpp">
...
When you specify an argument _by value_ in the function signature, you are asking for a copy of that argument.
