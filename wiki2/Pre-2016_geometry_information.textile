...
h1. Pre-2016 geometry information

Pictures for this information can be found at: https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/Geometry/

Up to date Geometry information can be found at: https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/Geometry_Package

The maintainer of this package is Gianluca Petrillo, petrillo@fnal.gov.
...
The key class within the Geometry package is geo::Geometry. This is a general interface into the detector descriptions of the different experiments. It is an [[Using_the_Framework#artServiceHandle|art::ServiceHandle]]. The idea is that none of the other packages have to explicitly depend on which detector is being considered; the detector-specific information is contained within the Geometry package.

There&#39;s more on [[Creating_GDML_|creating GDML]] files if you want to adjust or vary the geometry; e.g., for detector-design studies. However, you should *not* alter the number of volumes in the GDML files as that may have undesirable consequences on the running of the code.  If you think you need to do that for some reason, please contact Brian Rebel (brebel@fnal.gov) to see if an alternate solution is possible.

h2. Gettting the Geometry
...
h2. Code APA Construction (Channels and Wires)
...
h2. Detector Specific Methods (Volume Sorting and Channel Mapping)
...
h3. Standard Configuration: "ChannelMapStandardAlg":http://nusoft.fnal.gov/larsoft/doxsvn/html/classgeo_1_1ChannelMapStandardAlg.html
...
h3. APA Configuration: "ChannelMapAPAAlg":http://nusoft.fnal.gov/larsoft/doxsvn/html/classgeo_1_1ChannelMapAPAAlg.html

Detailed description of sorting and mapping pending.

h3. 35t Configuration: "ChannelMap35Alg":http://nusoft.fnal.gov/larsoft/doxsvn/html/classgeo_1_1ChannelMap35Alg.html
...
h2. Accessing coordinate information
...
_(note that there are better ways to find the plane pitch, the best being "asking the TPC":http://nusoft.fnal.gov/larsoft/doxsvn/html/classgeo_1_1TPCGeo.html#a697df19a5c945ce5622c50f80e60fbce )_
...
h2. Accessing other detector information
...
h2. Geometry Volume Hierarchy GDML

h3. Geometry Object Hierarchy
...
h3. Developing Geometry Volume Hierarchy
...
When running simulation, one can use the following volumes:
  ...
  volWorld
    Includes surrounding cement, 10 foot dirt overburden, surrounding dirt.

p=. !{width: 30%}volWorld.png!

  volDetEnclosure
    Includes everything inside of detector enclosure---not including the cement or overburden. 
    Currently, this is a cylindrical chamber with radius=48&#39;8" and height=25&#39;6" that contains 
    only air and volCryostat.

  volCryostat
    Includes PMTs, contents of volTPC, cryostat shell.

p=. !{width: 30%}volCryostat.png!

  volTPC
    Includes field cage, wire planes, cathode, top ground plate.

p=. !{width: 30%}volTPC.png!


h2. Viewing Volumes in ROOT GL Viewer
...
h2. TPC and Cryostat specifications

p=. !{width: 30%}volTPC_xy.png!

p=. !{width: 30%}volTPC_yz.png!
...
Our coordinate system is defined from the front edge of volTPC (see "origin of volWorld" in the volTPC figure). The block of liquid argon volume that is contained by the field cage, the anode plane and the innermost wire plane of the TPC are defined by its edges as marked by the dotted blue line in the above visuals. This block of argon will be denoted as the "chamber" in the following specifications:
  ...
  Chamber in volWorld (simulation) coordinates (cm):
    x  =  2.97500000000001  to      x  =  252.9
    y  =  -102.7            to      y  =  102.7
    z  =  87.3  	    to      z  =  1112.7

  Chamber in volTPC* coordinates (cm):
    x  =  -127.025      to      x  =  122.9
    y  =  -102.7        to      y  =  102.7
    z  =  -512.7        to      z  =  512.7

  Chamber Volume  =  52.638493713 m^3
  Chamber Mass    =  73693.8911982 kg

 LAr mass in volCryostat       =  225 226 kg
    Total  volCryostat mass      =  261 012 kg
    Non-LAr volCryostat mass       =   35 785.2 kg

  LAr mass in volTPC               =  112 933 kg
    Total volTPC mass            =  115 150 kg
    Non-LAr volTPC mass            =    2 217.69 kg

  Total wires in Y Plane        =  3457 wires
  Total wires in each UV Plane  =  2400 wires
