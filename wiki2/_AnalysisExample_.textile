...
h1. Getting started with an analysis task
...
The art/LArSoft framework is actively evolving. It&#39;s possible that some of the steps below no longer work as is (everything has been verified as of July-2017). If something goes wrong, the following resources may help:
    ...
    * [[ Quick-start guide to using and developing LArSoft code |LArSoft Quick-Start Guide]]
    * [[mrb:MrbUserGuide|MRB user guide]]

This page covers much the same ground as the [[ Quick-start guide to using and developing LArSoft code |quick-start guide]], but this tutorial is focused on analysis tasks, and it explains the reason why everything works as we go along. Hopefully you&#39;ll have enough clues to solve any problems that come up.

h2. Set up the LArSoft environment

You have to set up the LArSoft development environment for your installation. Look up the instructions for your location; e.g. [[_Quick-start_guide_to_using_and_developing_LArSoft_code_|Fermilab]] or "Nevis":https://twiki.nevis.columbia.edu/twiki/bin/view/Main/LArSoftAtNevis.

h2. Set up a working directory

This is described in the "quick-start guide":https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/_Quick-start_guide_to_using_and_developing_LArSoft_code_. You&#39;ll typically be working with files created by a specific release of LArSoft. 
...
h2. Checkout the AnalysisExample package to your development directory

The best way to get AnalysisExample and its associated documentation is by checking out the @larexamples@ product from the repository. (As you will learn as you work with LArSoft, you can only checkout code on the "product" level; you cannot just checkout an individual package.) This is covered in the "quick-start guide":https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/_Quick-start_guide_to_using_and_developing_LArSoft_code_. I&#39;m going to do this with variables, as I did above:
...
h3. The @larexamples@ version tag
...
h2. Copying and renaming AnalysisExample to ${YourTask}

_Warning:_ At this point, you may be tempted to go into @${MRB_SOURCE}/larexamples/larexamples/AnalysisExample@ and start making changes. *Don&#39;t do this! No!* 
...
h4. Doing a complete generation/simulation/reconstruction test
...
"Using ART in LArSoft":https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/Using_art_in_LArSoft
"Doxgen LArSoft documentation":http://nusoft.fnal.gov/larsoft/doxsvn/html/
"Running jobs":https://cdcvs.fnal.gov/redmine/projects/larsoft/wiki/_Running_Jobs_
