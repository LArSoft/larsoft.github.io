...
h1. LArSoft Release Official Build

{{TOC}}

h2(count). Platforms

See the [[_Supported_platforms_|Supported Platform]] list for full details.
...
h2(count). Build larsoft from the new tag

* This step does not use mrb

h3(count). run the Jenkins build
...
h3(count). Copy files from Jenkins
...
* install larutils <pre>vagrant ssh
mkdir devel
cd devel
git clone http://cdcvs.fnal.gov/projects/larutils</pre>
* build in a working directory <pre>vagrant ssh
mkdir build
cd build
...
h2(count). Make the binary build available on SciSoft
