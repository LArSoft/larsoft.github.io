...
h1. Time profiling tools

{{toc}}

h2. valgrind (@callgrind@ tool)
...
But, it&#39;s _valgrind-slow_: @valgrind@ is sort of a processor emulator, which interprets each machine instruction of the original program.
...
<pre>igprof --performance-profiler --target lar --compress --output CONFIG-DATE-performance.gz --debug lar -c CONFIG.fcl -s INPUT.root</pre>
...
bq. This profiler is not currently maintained.
...
The only limit is that it has not been updated in a while, and that could affect the efficiency of the stack unwinding[1] on code created with recent compilers.

fn1. Stack unwinding is the discovery of the full chain of calls of the current function.
