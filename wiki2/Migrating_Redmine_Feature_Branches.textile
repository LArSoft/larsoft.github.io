...
h1(count). Moving a Legacy Redmine Feature Branch to GitHub
...
h2(count).  Special cases

h3(count).  What if I have more than one feature branch?
...
h3(count).  What if there is a fork already existing for your github username?
...
h4(count).  Update from the primary

<pre>
git pull upstream
</pre>

h4(count). Delete the orphaned fork and starting fresh
