...
The "git flow":https://github.com/nvie/gitflow tool is a set of git extension that provide high-level operations for working within this branching model. We recommend that developers utilize @git flow@ when developing LArSoft code. Please consult the [[cet-is-public:git flow quick start|git flow quick-start guide]] for information on using git flow. Developers who choose not to use @git flow@ should nonetheless adhere to the branching model.
