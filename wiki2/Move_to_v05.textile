...
h2. Pass 2 - Core Service changes
...
| %{color: green}@        modified:   lardata/AnalysisAlg/CalorimetryAlg.cxx@%                 | core service changes happened on top of the refactored file: this implies the file has not changed since @v05_00_00_rc@ used it to apply core service changes |
| %{color: green}@        new file:   lardata/DetectorInfo/DetectorClocksException.h@%         | the file was added by the core service changes (or moved by them, that for @git@ is a new file and a file detection; see next item) |
| %{color: red  }@        both deleted:    Utilities/timeservice.fcl@%                         | the file was deleted in both branches, likely an effect of the refactoring (or moved: as above) |
| %{color: red  }@        both modified:   lardata/AnalysisAlg/CMakeLists.txt@%                | a conflict on an existing file: file needs editing |
| %{color: red  }@        both added:      lardata/CMakeLists.txt@%                            | a conflict on a "new" file: file needs editing |
| %{color: red  }@        added by them:   lardata/DetectorInfo/ClockConstants.h@%             | added (or moved) by the core service changes; see lessons learned! |
| %{color: red  }@        added by us:     lardata/DetectorInfo/DetectorPropertiesStandard.h@% | new file, likely from addition to @develop@ after @v04_30_00@ |
...
# it&#39;s time to fix @CMakeLists.txt@ files.
...
# at this point, @mrb build -C@ should succeed, with @cmake@ doing all the right things
# then, time to resolve the conflicts where %{color: red}@both@% modified or added files; most often, the conflict is just in @#include@ directives.
# finally, address the hidden conflicts of files that @git@ mistreated (see lessons learned below)
...
But keep in mind that the _good_ solution is to have your algorithm receive that service provider in a setup phase (@void Setup(detinfo::DetectorProperties const* detprop) { fDetProp = detprop; }@ - or you can use a @ProviderPack@) from the controlling module or algorithm.
...
<pre>INFO:     pattern &#39;\bUtilities/DetectorProperties.h\b&#39; matched at larreco/TrackFinder/PMAlgTrackMaker_module.cc@50:
...
INFO: 2 file would be changed under &#39;larreco/TrackFinder/PMAlgTrackMaker_module.cc&#39;</pre>
