...
h1. Using git repositories

LArSoft has specific guidelines for git code repositories.

h2(count).  Directory Structure

h3(count). Recommended Directory Structure
...
h3(count). Optional Subdirectories
...
h2(count).  Inappropriate Files

h3(count).  Large files
...
h3(count).  Other inappropriate files
