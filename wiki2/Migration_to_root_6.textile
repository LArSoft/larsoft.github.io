...
h1. Migration to root 6

{{>TOC}}

h1. Release Candidates rc4, rc5, and rc6
...
* Changes are provided for the the experiment code, but users may need to run [[Migration_to_root_6#UpdateToLArSoftObjsh|UpdateToLArSoftObj.sh]]

h2. v06_00_00_rc5 is in sync with larsoft v05_13_00

h2. Guide to new products
...
h1. Release Candidates rc1, rc2, and rc3
...
h2. v06_00_00_rc3 is in sync with larsoft v05_11_01

h2. v06_00_00_rc2 is in sync with larsoft v05_10_00

h2. v06_00_00_rc1 is in sync with larsoft v05_08_00

h1.  Known issues and changes

h2. gdml breaking change
...
h2. root dictionaries

h3. ROOT_INCLUDE_PATH
...
---- DictionaryNotFound END</pre>
* Add the line to the appropriate dictionary:
<pre><enum name="geo::_plane_sigtype"                                         /></pre>
...
h2. @TFormula@ discovered integral math
...
h2. @TFormula@ bug on missing parameter in expression
...
|/2. @"[0]*x**[2]"@ |/2. @"[0]*x**[1]"@ | @SetParameters(0., 1., 2.)@ results in @[2] <== 1.@ |
|   @SetParameter(2, 2.)@ has no effect (there is no parameter 2) |
|/3. @gaus(0) + [4]**[5]@ |/3.  @gaus(0) + [3]**[4]@ | @SetParameters(0., 1., 2., 3., 4., 5.)@ results in @[4] <== 3.@,  @[5] <== 4.@ |
...
<pre>setup larsoft v05_09_00 or later
cd to the gdml directory
RemoveMathFromGDML.py *.gdml
the original gdml files will be named xxx.gdml.bak</pre>

h3. verification of  RemoveMathFromGDML.py
