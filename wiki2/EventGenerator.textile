...
# CRY - cosmic ray generation
# GENIE - neutrino interaction generation
# Single - creation of predefined particles at given energies, directions in the detectors
# LightSource - photon generation for PMT studies
...
h2. Cosmic Ray Generation with CRY
...
h2. Neutrino Interaction Generation with GENIE
...
The default behavior is for GENIE to produce events in accordance with a spill intensity of 50e12 POT. If you instead wish to produce a predefined number, _n_, interactions in a spill you have to set the EventsPerSpill parameter to be _n_ instead of 0 (the default that signals the use of POT per spill instead). 

To create a file containing neutrino interactions for each event, do

% lar -c job/prodgenie.fcl

h3. Flux files available for use
...
h3. Note about Mother and daughters in GENIE and LArSoft
...
h2.  Light Source Photon Generation
...
h1. A Word about Co-ordinate Systems

As noted in the discussion on co-ordinate systems on the [[Geometry]] page, the origin is not necessarily located in the center of the upstream end of the TPC. 

h1. List of Parameters
