...
If this pattern is uniform and repeated enough, supporting utilities should be written to make it straightforward<sup>[1](#fn1)</sup>.

<sup>1</sup> One such attempt for the specific use case of hit creation is `recob::HitCollectionCreator` (`lardata/lardata/RecoBaseArt/HitCreator.h`), which might be defined “moderately successful” in that I received a single unsolicited fairly positive statement and no negative statement about its usefulness.
