...
The service interface is a (possibly abstract<sup>[1](#fn1)</sup>) class that describes all the service is expected to be able to do. For example<sup>[2](#fn2)</sup>:
...
<sup>1</sup> An abstract class, in C++ sense, is a class that has virtual methods it does not provide a definition of. Derived classes must provide such definitions.

<sup>2</sup> Some LArSoft utilities enforce the recommendation of having service provider classes non-copiable and unmovable. The derivation from `lar::UncopiableAndUnmovableClass` achieves that goal.
