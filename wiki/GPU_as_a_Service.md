...
In the rest of Part 1 below, we provide a concrete example of using the NuSonic Triton Client API to send an inference request to the GPU server and retrieve the results. In [GPU as a Service Part 2](GPU as a Service part two), we explain how to set up an inference model on the GPU server. Finally, in [GPU as a Service Part 3](GPU as a Service part three), we describe how to test the client and model configuration locally using an inference server running in a docker container, or via a dedicated test server at Fermilab.
